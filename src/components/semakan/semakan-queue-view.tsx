...
interface SemakanQueueViewProps {
  initialQueue: ReviewQueueItemData[]
}

export function SemakanQueueView({ initialQueue }: SemakanQueueViewProps) {
  const [queue] = useState<ReviewQueueItemData[]>(initialQueue)
  ...
  const columns: TableColumnDef<ReviewQueueItemData>[] = [
    {
      id: 'referenceNo',
      headerMs: 'No. Rujukan',
  ...
  const filteredData = useMemo(() => {
    return queue.filter((item) => {
      ...
      const valA = a[sortState.column as keyof ReviewQueueItemData] ?? ''
      const valB = b[sortState.column as keyof ReviewQueueItemData] ?? ''
      const cmp = typeof valA === 'number' && typeof valB === 'number'
        ? valA - valB
        : String(valA).localeCompare(String(valB))
      return sortState.direction === 'asc' ? cmp : -cmp
    })
  }, [queue, filterState, sortState])
