...
import { STATUS_OPTIONS } from '../dashboard/recent-applications-table'
...
interface PermohonanListViewProps {
  initialApplications: ApplicationRowItem[]
}

export function PermohonanListView({ initialApplications }: PermohonanListViewProps) {
  const [applications] = useState<ApplicationRowItem[]>(initialApplications)
  ...
  const [paginationState, setPaginationState] = useState<TablePaginationState>({
    pageIndex: 0,
    pageSize: 10,
    totalRows: initialApplications.length,
  ...
  const columns: TableColumnDef<ApplicationRowItem>[] = [
    {
      id: 'referenceNo',
      headerMs: 'No. Rujukan',
  ...
  const filteredData = useMemo(() => {
    return applications.filter((row) => {
      ...
      const valA = a[sortState.column as keyof ApplicationRowItem] ?? ''
      const valB = b[sortState.column as keyof ApplicationRowItem] ?? ''
      const cmp = String(valA).localeCompare(String(valB))
      return sortState.direction === 'asc' ? cmp : -cmp
    })
  }, [applications, filterState, sortState])
